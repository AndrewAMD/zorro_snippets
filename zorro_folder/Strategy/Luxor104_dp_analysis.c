...
int dpMaxSteps(PARAMETER *p1, PARAMETER *p2, int assetsCounter)
{
	if(assetsCounter==0) assetsCounter = 1;
	return((p1->nMaxSteps) * (p2->nMaxSteps) * assetsCounter);
}

void dpGet(int nIndex, var *v1, var *v2, int *n1, int *n2, PARAMETER *p1, PARAMETER *p2, int assetsCounter)
{
	bool getAsset = false;
	if(assetsCounter) getAsset = true;
	int nTotalMaxSteps = dpMaxSteps(p1, p2, assetsCounter);
	if(assetsCounter==0) assetsCounter = 1;
	int i = 0, j = 0, k = 0, l = 0;
...
int dpIterate(PARAMETER *p1, PARAMETER *p2, int assetsCounter)
{
	if(!assetsCounter) assetsCounter = 1;
	int nTotalMaxSteps;
	nTotalMaxSteps	= dpMaxSteps(p1, p2, assetsCounter);
#ifdef litec_h
...
string g_dpAssetList[1000];
int g_dpAddAssetCounter = 0;
void dpAddAsset(string sAsset)
{
	if(is(FIRSTINITRUN))
	{
		g_dpAssetList[g_dpAddAssetCounter] = sAsset;
		g_dpAddAssetCounter++;
	}
	return;
}
...
var dp1Get()
{
	var v1 = 0., v2 = 0.;
	int n1 = 0, n2 =0;
	dpGet(g_dpIndex, &v1, &v2, &n1, &n2, &g_dp1, &g_dp2, g_dpAddAssetCounter);
	return(v1);
}
var dp2Get()
{
...
void dpIterate()
{
	set(PARAMETERS);
	dpInitMetrics();
	g_dpIndex = dpIterate(&g_dp1, &g_dp2, g_dpAddAssetCounter);
	return;
}
